...
function init() {
  ...
  document.addEventListener('click', e => {
    ...
    if (!card) return;
    const p = PERFUMES.find(x => x.id === card.dataset.id);
    if (p) openModal(p);
  });
}
...
function cardHTML(p, i) {
  ...
  return `
<article class="card" data-id="${esc(p.id)}" data-brand="${esc(p.marca)}" style="transition-delay:${delay}">
...
function setupModal() {
  ...
  overlay.innerHTML = `
    <div class="pmodal">
      <button class="pmodal-close" id="pmodalClose" aria-label="Cerrar">✕</button>
      <div class="pmodal-img" id="pmodalImg"></div>
      <div class="pmodal-body">
        <p class="pmodal-brand" id="pmodalBrand"></p>
        <h3 class="pmodal-name"  id="pmodalName"></h3>
        <p class="pmodal-price"  id="pmodalPrice"></p>
        <a class="pmodal-wa" id="pmodalWa" target="_blank" rel="noopener">
...
function openModal(p) {
  ...
  document.getElementById('pmodalBrand').textContent = p.marca;
  document.getElementById('pmodalName').textContent  = p.nombre;
  if (agotado) {
    priceEl.textContent = 'Agotado';
    priceEl.className = 'pmodal-price pmodal-price--na';
  } else {
    priceEl.textContent = `$${p.precio % 1 === 0 ? p.precio : p.precio.toFixed(2)}`;
    priceEl.className = 'pmodal-price';
  }
  document.getElementById('pmodalImg').innerHTML = p.foto
